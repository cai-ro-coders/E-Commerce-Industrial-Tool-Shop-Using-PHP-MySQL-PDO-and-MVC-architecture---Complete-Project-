<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted small mb-0"><?= sanitize($category['name']) ?></p>
    </div>
...
<form action="<?= $base_url ?>admin/categories/update/<?= $category['id'] ?>" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
    <?= csrf_field() ?>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control" id="name" name="name" value="<?= sanitize($category['name']) ?>" required>
                        </div>

                        <div class="col-12">
                            <label for="slug" class="form-label">Slug</label>
                            <input type="text" class="form-control" id="slug" name="slug" value="<?= sanitize($category['slug']) ?>">
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="4" placeholder="Brief description of this category"><?= sanitize($category['description']) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    ...
                    <label for="image" class="form-label"><?= !empty($category['image']) ? 'Replace Image' : 'Category Image' ?></label>
                    <input type="file" class="form-control" id="image" name="image" accept="image/jpeg,image/png,image/gif,image/webp">
                    <div class="form-text"><?= !empty($category['image']) ? 'Leave empty to keep existing image.' : 'Recommended: 400x400px.' ?></div>
